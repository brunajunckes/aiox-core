...
synology_dsm_deploy() {
  ...
  if [ -n "$SYNO_TOTP_SECRET" ]; then
    ...
    response=$(_post "method=login&account=$encoded_username&passwd=$encoded_password&api=SYNO.API.Auth&version=$api_version&enable_syno_REDACTED
    _debug3 response "$response"
  ...
  # Notice that if SYNO_USE_TEMP_ADMIN=1, both variables will be unset
    ...
    # Assume the current account disabled 2FA-OTP, try to log in right away.
      ...
      response=$(_get "$_base_url/webapi/$api_path?api=SYNO.API.Auth&version=$api_version&method=login&format=sid&account=$encoded_username&passwd=$encoded_password&enable_syno_REDACTED
  ...
  response=$(_post "$content" "$_base_url/webapi/entry.cgi?api=SYNO.Core.Certificate&method=import&version=1&SynoREDACTED
  _debug3 response "$response"
