...
dns_dpi_rm() {
  ...
  if ! _rest POST "Record.List" "login_REDACTED
  ...
  if ! _rest POST "Record.Remove" "login_REDACTED
...
add_record() {
  ...
  if ! _rest POST "Record.Create" "login_REDACTED
    return 1
  fi
...
_get_root() {
  ...
  while true; do
    ...
    if ! _rest POST "Domain.Info" "login_REDACTED
      return 1
    fi
