...
dns_west_cn_rm() {
  ...
  if ! _rest POST "domain/dns/" "act=dnsrec.list&username=$WEST_Username&REDACTED
  ...
  if ! _rest POST "domain/dns/" "act=dnsrec.remove&username=$WEST_Username&REDACTED
...
add_record() {
  ...
  if ! _rest POST "domain/dns/" "act=dnsrec.add&username=$WEST_Username&REDACTED
    return 1
  fi
