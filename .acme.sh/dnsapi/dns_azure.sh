...
dns_azure_add() {
  ...
  if [ -z "$AZUREDNS_SUBSCRIPTIONID" ]; then
    ...
    AZUREDNS_TENANTID=""
    AZUREDNS_APPID=""
    AZUREDNS_CLIENTREDACTED
    AZUREDNS_BEARERREDACTED
...
_azure_getaccess_token() {
  ...
  if [ "$managedIdentity" = true ]; then
    ...
    body="resource=$(printf "%s" 'https://management.core.windows.net/' | _url_encode)&client_id=$(printf "%s" "$clientID" | _url_encode)&client_REDACTED
