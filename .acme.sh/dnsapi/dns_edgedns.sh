...
_EDGEDNS_credentials() {
  ...
  args_missing=0
  AKAMAI_ACCESS_REDACTED
  if [ -z "$AKAMAI_ACCESS_TOKEN" ]; then
    AKAMAI_ACCESS_REDACTED
    AKAMAI_CLIENT_REDACTED
    AKAMAI_HOST=""
    AKAMAI_CLIENT_REDACTED
...
_edgedns_make_auth_header() {
  ...
  _edgedns_new_nonce
  _edgedns_eg_timestamp
  # "Unsigned authorization header: 'EG1-HMAC-SHA256 client_REDACTED
  _auth_header="$(printf "EG1-HMAC-SHA256 client_REDACTED
