...
dns_internetbs_add() {
  ...
  # https://testapi.internet.bs/Domain/DnsRecord/Add?REDACTED
...
dns_internetbs_rm() {
  ...
  # https://testapi.internet.bs/Domain/DnsRecord/List?REDACTED
  ...
  if _contains "$response" "\name\":\"${_sub_domain}.${_domain}\""; then
    ...
    # https://testapi.internet.bs/Domain/DnsRecord/Remove?REDACTED
...
_get_root() {
  ...
  # https://testapi.internet.bs/Domain/List?REDACTED
...
_internetbs_rest() {
  ...
  if [ "$m" = "GET" ]; then
    response="$(_get "${url}?REDACTED
  else
    _debug2 data "$data"
    response="$(_post "$data" "${url}?REDACTED
  fi
