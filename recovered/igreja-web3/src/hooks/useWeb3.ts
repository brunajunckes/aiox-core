...
const INITIAL_STATE: UseWeb3State = {
  provider: null,
  account: null,
  balance: '0',
  isConnecting: false,
...
export const useWeb3 = () => {
  const [state, setState] = useState<UseWeb3State>(INITIAL_STATE)
  ...
  const getSafeInfo = useCallback(
    async (safeAddress: string): Promise<{ address: string; balance: string } | null> => {
      if (!state.provider) {
        setState(prev => ({
          ...prev,
