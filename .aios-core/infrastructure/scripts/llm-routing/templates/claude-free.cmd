...
for /f "usebackq eol=# tokens=1,* delims==" %%a in ("%PROJECT_ROOT%\.env") do (
    if "%%a"=="DEEPSEEK_API_KEY" set "DEEPSEEK_REDACTED
)
...
endlocal & set "ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic" & set "ANTHROPIC_REDACTED
